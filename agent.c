...
jvmtiEnv* _env = NULL;
jlong _id = 0;
jvmtiExtensionFunction _request_jfr_stack_trace = NULL;

static void handler(int signo, siginfo_t* info, void* context) {
  if (_request_jfr_stack_trace != NULL) {
    jvmtiError err = _request_jfr_stack_trace(_env, NULL, context, _id++);
    if (err != JVMTI_ERROR_NONE) {
      printf("Error in RequestJFRStackTrace: %d\n", err);
    }
  }
}
...
void ThreadStart(jvmtiEnv* jvmti_env, JNIEnv* jni_env, jthread thread) {
  jvmtiError error;
    ...
    // Create a per-thread CPU-time timer.
    timer_t timerid;
    struct sigevent sev;
...
jint Agent_OnLoad(JavaVM* vm, char* options, void* reserved) {
  ...
  for (jint i = 0; i < extension_count; i++) {
    ...
    if (strcmp(ext_info->id, "com.sun.hotspot.functions.RequestJFRStackTrace") == 0) {
      _request_jfr_stack_trace = ext_info->func;
    }
